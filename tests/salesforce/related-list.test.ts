...
describe('parseContactRelatedOppsFromDom', () => {
  it('extracts opp id + name from rows with /Opportunity/<id> anchors', () => {
    document.body.innerHTML = `
      <div>
        <a href="/lightning/r/Opportunity/006AA0001/view"><span>Acme Q3 Renewal</span></a>
        <a href="/lightning/r/Opportunity/006AA0002/view"><span>Beta Expansion</span></a>
      </div>
    `;
    const rows = parseContactRelatedOppsFromDom(document);
    expect(rows.length).toBe(2);
    expect(rows[0].id).toBe('006AA0001');
    expect(rows[0].name).toBe('Acme Q3 Renewal');
    expect(rows[1].id).toBe('006AA0002');
  });

  it('skips anchors whose href does not match an Opportunity record', () => {
    document.body.innerHTML = `
      <div>
        <a href="/lightning/r/Opportunity/006AA0001/view"><span>Acme</span></a>
